...
messages also carry a *second* line inside their text: `check_match` names the line of the
token being closed, and `read_long_string` names the line the bracket opened on. That inner
line differs from the prefix line, and both are asserted by the suite.
...
Three groups, recorded so their absence from the parse scoreboard is deliberate rather than
unnoticed.
